...
mlr --csv head -n 100 then put -S '$rssDate = strftime(strptime($created_at, "%a %b %d %H:%M:%S +0000 %Y"),"%Y-%m-%dT%H:%M:%SZ")' then cut -o -f titolo,URL,rssDate then label title,link,pubDate then put '$guid=$link' "$folder"/processing/archive_ita.csv >"$folder"/rawdata/data_ita.csv
